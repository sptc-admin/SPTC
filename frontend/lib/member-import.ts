...
import type { MemberCreatePayload } from "@/lib/members-api"
import {
...
import { DEFAULT_PROFILE_IMAGE, STATIC_MEMBER_FINANCIALS } from "@/lib/member-types"
import { triggerXlsxDownload } from "@/lib/csv-export"

export const MEMBER_IMPORT_SHEET = "Members"
...
function buildHeaderMap(headerRow: unknown[]): Map<string, string> {
  ...
  for (const [field, labels] of Object.entries(HEADER_SYNONYMS)) {
    for (const label of labels) {
      keyByCol.set(normHeader(label), field)
    }
  }
...
export type MemberImportRowError = { excelRow: number; message: string }

export type MemberImportParsedRow = {
  excelRow: number
  payload: MemberCreatePayload
}

export type MemberImportParseOptions = {
  existingBodyNormLower?: Set<string>
}

export function buildMemberImportTemplateBuffer(): ArrayBuffer {
  const memberHeaders = [
    "Body #",
    "Precinct #",
    "First name",
  ...
  const memberRows = [memberHeaders, sampleRow]
  const wsMem = XLSX.utils.aoa_to_sheet(memberRows)
  wsMem["!cols"] = memberHeaders.map((h) => ({
    wch: Math.min(48, Math.max(14, h.length + 4)),
  }))
  XLSX.utils.book_append_sheet(wb, wsMem, MEMBER_IMPORT_SHEET)

  return XLSX.write(wb, { bookType: "xlsx", type: "array", compression: true })
}

export function downloadMemberImportTemplate(): void {
  const buf = buildMemberImportTemplateBuffer()
  triggerXlsxDownload("members-import-template", buf)
}

export function parseMemberImportRows(
  arrayBuffer: ArrayBuffer,
  options?: MemberImportParseOptions,
): {
  items: MemberImportParsedRow[]
  errors: MemberImportRowError[]
} {
  const wb = XLSX.read(arrayBuffer, { type: "array", cellDates: true })
  const sheetName =
    wb.SheetNames.find((n) => n.trim().toLowerCase() === MEMBER_IMPORT_SHEET.toLowerCase()) ??
    wb.SheetNames[0]
  if (!sheetName) {
    return {
      items: [],
  ...
  const items: MemberImportParsedRow[] = []
  ...
  const bodyNumbersSeenInFile = new Set<string>()
  ...
  for (let i = 1; i < matrix.length; i++) {
    ...
    const bodyNormKey = normalizeBodyNumber(bodyNumber).toLowerCase()
    if (bodyNumbersSeenInFile.has(bodyNormKey)) {
      pushErr("Duplicate Body # in this file.")
      continue
    }
    if (options?.existingBodyNormLower?.has(bodyNormKey)) {
      pushErr("A member with this Body # already exists.")
      continue
    }
    bodyNumbersSeenInFile.add(bodyNormKey)
    if (!precinctNumber) {
    ...
    items.push({
      excelRow,
      payload: {
        bodyNumber: normalizeBodyNumber(bodyNumber),
        precinctNumber,
        ...
        financials: { ...STATIC_MEMBER_FINANCIALS },
      },
    })
  }
  ...
  if (items.length === 0 && errors.length === 0) {
    if (
      ...
      errors.push({
        ...
        message: `Row ${firstSkippedSampleExcelRow ?? 2} is still the template sample (Body # contains SAMPLE-DELETE). Replace it with a real member or add more rows below it, then import again.`,
      })
    } else {
      errors.push({
        excelRow: 0,
        message:
          "No data rows found under the header row. Enter at least one member starting on row 2.",
      })
    }
  }
